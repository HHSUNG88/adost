...
/**
 * This class demonstrates using HALogin v2
 */
public class HALogin_v2 {
  ...
  public static void usage() {
    println("");
    println("java ...HALogin_v2 -source <sourceID> -target <targetID> " + "[-password <password>] "
        + "[-adminslot <adminSlot>] " + "[-usekey <useKey>] " + "[-deleteobjects] <deleteObjects>");
    println("");
    ...
    println("<useKey> use privatekey(true) or use PKC chain(false)");
  ...
  public static void main(String[] args) {
    ...
    int[] users = { (int) CKU.USER.longValue() };
    int adminTokenType = 0x10000000;// CKF_ADMIN_TOKEN = 0x10000000
    int userTokenType = 0;
    int[] tokenTypes = { userTokenType };
    long numRoles = 1; // or could be LUNA.HA_ALL_ROLES_ALLOWED

    boolean useKey = true;
    boolean adminSlot = false;
    final boolean clearHARecovery = true;
    // For HA v1.1, assume v0 partitions as both primary and secondary.
    // Otherwise would need to create public key on secondary
    // to avoid cloning between v0 and v1 partitions.
    final boolean HA_v1_1 = false;
    ...
    for (int i = 0; i < args.length; ++i) {
      ...
      } else if (args[i].equalsIgnoreCase("-usekey")) {
        if (++i >= args.length) {
          usage();
        }
        useKey = Boolean.parseBoolean(args[i]);
    ...
    try {
      ...
      if (deleteObjects) {
        ...
        if (adminSlot) {
          ...
          tokenTypes[0] = adminTokenType;
          users[0] = (int)CKU.SO.longValue();
          tokenTypes[0] = adminTokenType;
        ...
        CK_ATTRIBUTE[] temp_rsaPriv4096 = { new CK_ATTRIBUTE(CKA.LABEL, label_rsaPriv4096.getBytes()),
            ...
            new CK_ATTRIBUTE(CKA.SIGN, CK_BBOOL.FALSE), new CK_ATTRIBUTE(CKA.MODIFIABLE, CK_BBOOL.FALSE) };
        ...
        if (sourcePrivateKey.longValue() == CK.INVALID_HANDLE) {
          ...
          CryptokiEx.C_GenerateKeyPair(sourceSession, new CK_MECHANISM(CKM.RSA_PKCS_KEY_PAIR_GEN), temp_rsaPub4096,
              temp_rsaPub4096.length, temp_rsaPriv4096, temp_rsaPriv4096.length, jcprov_rsaPub4096, jcprov_rsaPriv4096);
        ...
        if (!HA_v1_1 && clearHARecovery) {
          CryptokiEx.CA_HAInitExtended(targetSession, null, null, 0, null, null, 0);
        }
        ...
        if (HA_v1_1) {
          ...
          if (useKey) {
            // use key
            CryptokiEx.CA_HAInitExtended(targetSession, targetPrivateKey, null, // no PKC provided
                0, // no PKC provided
                users, tokenTypes, numRoles);

          } else {
            // use PKC chain
            CK_ATTRIBUTE[] template = { new CK_ATTRIBUTE(CKA.LUNA_ATTR_PKC_CITS, null, 0) };
            CryptokiEx.C_GetAttributeValue(targetSession, targetPrivateKey, template, template.length);
            template[0].pValue = new byte[(int) template[0].valueLen];
            CryptokiEx.C_GetAttributeValue(targetSession, targetPrivateKey, template, template.length);

            LongRef PKCChainLen = new LongRef(0);
            int PKCChainFinalLen = (int) template[0].valueLen;
            byte[] PKCChainDAC = null;
            CryptokiEx.CA_GetTokenCertificates(targetSlotId, CKA.CKHSC_CERT_TYPE_CITS_DAC.longValue(), PKCChainDAC,
                PKCChainLen);
            PKCChainDAC = new byte[(int) PKCChainLen.value];
            CryptokiEx.CA_GetTokenCertificates(targetSlotId, CKA.CKHSC_CERT_TYPE_CITS_DAC.longValue(), PKCChainDAC,
                PKCChainLen);
            System.out.println("PKCChainDAC.length:" + PKCChainDAC.length);
            PKCChainFinalLen += PKCChainDAC.length;
            PKCChainLen = new LongRef(0);
            byte[] PKCChainHOC = null;
            CryptokiEx.CA_GetTokenCertificates(targetSlotId, CKA.CKHSC_CERT_TYPE_CITS_MICHOC.longValue(), PKCChainHOC,
                PKCChainLen);
            PKCChainHOC = new byte[(int) PKCChainLen.value];
            CryptokiEx.CA_GetTokenCertificates(targetSlotId, CKA.CKHSC_CERT_TYPE_CITS_MICHOC.longValue(), PKCChainHOC,
                PKCChainLen);
            System.out.println("PKCChainHOC.length:" + PKCChainHOC.length);
            PKCChainFinalLen += PKCChainHOC.length;

            byte[] PKCChainFinal = new byte[PKCChainFinalLen];
            System.arraycopy(template[0].pValue, 0, PKCChainFinal, 0, (int) template[0].valueLen);
            System.arraycopy(PKCChainDAC, 0, PKCChainFinal, (int) template[0].valueLen, PKCChainDAC.length);
            System.arraycopy(PKCChainHOC, 0, PKCChainFinal, ((int) template[0].valueLen + PKCChainDAC.length),
                PKCChainHOC.length);

            System.out.println("-----BEGIN PKC CHAIN-----");
            // byte[] aNewB64 = java.util.Base64.getEncoder().encode(PKCChainFinal);
            // String b64 = new String(aNewB64);
            // System.out.println(b64);
            System.out.println(toHex(PKCChainFinal));
            System.out.println("-----END PKC CHAIN-----");

            CryptokiEx.CA_HAInitExtended(targetSession, new CK_OBJECT_HANDLE(0), // no key provided
                PKCChainFinal, PKCChainFinal.length, users, tokenTypes, numRoles);
          }
        }
        ...
        byte[] twc = null;
        if (HA_v1_1) {
          CryptokiEx.CA_HAGetMasterPublic_V1_1(sourceSession, null, twcLen);
          twc = new byte[(int) twcLen.value];
          CryptokiEx.CA_HAGetMasterPublic_V1_1(sourceSession, twc, twcLen);
        } else {
          CryptokiEx.CA_HAGetMasterPublicData(sourceSession, sourcePrivateKey, null, twcLen);
          twc = new byte[(int) twcLen.value];
          CryptokiEx.CA_HAGetMasterPublicData(sourceSession, sourcePrivateKey, twc, twcLen);
        }

        // Get the target challenge
        byte[] challengeBlob = null;
        if (adminSlot) {
           CryptokiEx.CA_HAGetLoginChallenge(targetSession, CKU.SO, twc, twcLen.value,
               null, challengeBlobLen);//admin partition
           challengeBlob = new byte[(int) challengeBlobLen.value];
           CryptokiEx.CA_HAGetLoginChallenge(targetSession, CKU.SO, twc, twcLen.value,
               challengeBlob, challengeBlobLen);//admin partition
        } else {
          CryptokiEx.CA_HAGetLoginChallenge(targetSession, CKU.USER, twc, twcLen.value,
              null, challengeBlobLen);
          challengeBlob = new byte[(int) challengeBlobLen.value];
          CryptokiEx.CA_HAGetLoginChallenge(targetSession, CKU.USER, twc, twcLen.value,
